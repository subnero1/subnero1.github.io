...
> This is a legacy product and is now replaced with the [multi-receiver modems]({{ site.baseurl }}/products/wnc/gen4x/accessories/multi-receiver.html){:target="_blank"}.
...
The time at which the signal is detected on the main hydrophone is extracted using `msg.rxTime`. The recording time can be set in the past while requesting the recording to synchronize the recorded data from the multi-receiver acquisition system with the signal recorded on the main hydrophone.
